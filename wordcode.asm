...
code segment
  Assume cs:code, ds:data, ss:stack  
    ...
    PGCD_I proc
       ...
       division:
       mov pgcd,ax ;le dernier dividende avant l'int 0 est le pgcd (dernier rest non nule)
       mov dx,0 ;car div va utilise le registre dx
       div bx
       Cmp done,1  ; si done=1 : l'int de devision par 0 a ete declenche
       jz fin
       mov ax,bx  ; le dividende recoit l'ancien diviseur 
       mov bx,dx  ; le diviseur recoit le rest  
       jmp division
       fin:
       pop dx  ; depiler l'@ de retour
       push pgcd ; empiler le pgcd
     ...
     PGCD_I endp
      ...
      PGCD_IN proc
       ...
       Mov si, 0 
       Mov ax, tab[si] ; ax recoit la 1ere val (le 1er dividende)

      Enc:                 
       add si,2 ;car les variables de tab sont de type word(2 cases memoire pour chaque val)
       Mov bx, tab[si] ; a chaque fois bx va recoit la valeur suivante
       Call PGCD_I  
